...
public function select($table, $join, $columns = null, $where = null)
{
    ...
    // Jeśli $columns to pojedynczy ciąg, przekształć go w tablicę
    if (is_string($columns)) {
        $columns = [$columns];  // Zamiana na tablicę
    }

    // Sprawdź, czy mamy tylko jedną kolumnę (np. "surname")
    if (count($columns) === 1 && isset($columns[0])) {
        $columns = [$columns[0]];  // Upewnij się, że mamy tablicę z jedną kolumną
    }

    // Teraz wywołaj kontekst zapytania z odpowiednio przetworzonymi kolumnami
